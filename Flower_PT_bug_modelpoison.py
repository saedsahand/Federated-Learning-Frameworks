...
def partition_training_data(X_train, Y_train, tokenizer, num_clients, valid_percent=0.1):
    ...
    valloaders = []
    for ds in datasets:
        len_val = int(len(ds) * valid_percent)  # 10 % validation set
        ...
        valid_dataset = CodeDataset(valid_codes, valid_labels, tokenizer)
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if self.device_ID == 1:
            ...
            set_noise_to_parameters(self.model, get_parameters(self.model))
...
def set_noise_to_parameters(net, parameters: List[np.ndarray]):
    
    items = [param+1 for param in parameters]
    
    params_dict = zip(net.state_dict().keys(), items)
    state_dict = OrderedDict({k: torch.Tensor(v) for k, v in params_dict})
    net.load_state_dict(state_dict, strict=True)    
